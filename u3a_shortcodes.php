...
//	write_log($atts);
	...
	if ($atts["group"])
	{
		...
		$closea = new U3A_A('#', $cls, 'u3a-find-member-a' . $idsuffix, null, "u3a_select_group_member_dialog_close('" . $atts["next_action"] . "', '" . $atts["op"] . "');");
	...
	else
	{
		...
		$closea = new U3A_A('#', $cls, 'u3a-find-member-a' . $idsuffix, null, "u3a_find_member_dialog_close('" . $atts["next_action"] . "', '" . $atts["op"] . "', '" . $atts["suffix"] . "');");
		$closea->add_attribute("rel", "modal:close");
	...
	$open = new U3A_A('#u3a-find-member-div' . $idsuffix, '<span class="dashicons dashicons-' . $atts["icon"] . '"></span>', 'u3a-find-member-search-a' . $idsuffix, "u3a-inline-block");
...
//		write_log($coords);
		...
		foreach ($coords as $cd)
		{
			if ($cd)
			{
				...
				$ml = new U3A_A("#u3a-send-mail-individual-0", $c->forename . " " . $c->surname, null, "u3a-group-mail-class", "u3a_mail_clicked('" . $c->email . "')");
...
//		usort($mbrs, ["U3A_Members", "compare"]);
	...
	if ($atts["op"])
	{
		...
		$group_input = new U3A_INPUT("hidden", "u3a-member-select-group", "u3a-member-select-group-" . $atts["op"], null, $groups_id);
		$groupname_input = new U3A_INPUT("hidden", "u3a-member-select-groupname", "u3a-member-select-groupname-" . $atts["op"], null, $group_name);
	}
	else
	{
		...
		$groupname_input = new U3A_INPUT("hidden", "u3a-member-select-groupname", "u3a-member-select-groupname", null, $group_name);
	}
//	write_log("members in $group");
//	write_log($mbrs);
	if ($atts["includecount"] === "yes")
	{
		$nmem = new U3A_DIV("Current membership: " . count($mbrs), null, "u3a-count-members-class u3a-border-bottom");
...
function u3a_delete_document_contents($atts1)
{
		...
		foreach ($alldocs["documents"] as $catname => $catdocs)
		{
			...
			if ($catdocs["count"])
			{
				...
				$lbl = [
					...
					$sel,
					new U3A_BUTTON("button", "delete", "u3a-" . $type_name . "-select-button", "u3a-select-button-class u3a-button", "u3a_delete_document('" . $selid . "', '" . $type_name . "')")
				];
			}
			else
			{
			...
			$div = new U3A_DIV($lbl, "u3a-delete-document-div-" . $grp . "-" . $type . "-" . $catid, "u3a-delete-document-div-class-$type u3a-border-top");
	...
	{
		$del[] = new U3A_DIV("No " . $type_name . "s to delete", "u3a-delete-documents-div-" . $grp . "-" . $type, "u3a-delete-document-div-class-$type u3a-border-top");
	}
...
function u3a_coordinators_contents($atts1)
{
	...
	if (U3A_Committee::is_committee_member($mbr) || U3A_Group_Members::is_a_coordinator($mbr))
	{
		...
		if (U3A_Committee::is_committee_member($mbr))
		{
			...
			$coorddiv = new U3A_DIV('[u3a_coordinators_list checked="yes"]', "u3a-coordinators-div-0", "u3a-coordinators-list-class u3a-inline-block u3a-padding-right-5 u3a-width-30-pc u3a-va-top");
			$emaildiv1 = U3A_HTML_Utilities::get_mail_contents_div($mbr->id, "coordinators", 0, "u3a-inline-block u3a-width-70-pc u3a-height-100-pc u3a-va-top", 0, 0);
			$pgcontent .= '[su_tab title="Email" disabled="no" anchor="" url="" target="blank" class=""]' . "\n" . U3A_HTML::to_html([$coorddiv, $emaildiv1]);
		}
		if (U3A_Committee::is_webmanager($mbr))
		{
			...
			$dlbtn = new U3A_BUTTON("button", "download as csv", "u3a-group-emails-download", "u3a-wide-button", "u3a_download_coordinator_emails()");
		...
		foreach ($coords as $mbr)
		{
			if ($seeall || $mbr["membership_number"] < 100000)
			{
				if ($checked)
				{
					...
					$cblbl = new U3A_LABEL($cbid, $mbr["group_name"] . ", ", null, "u3a-member-checkbox-label-class u3a-inline-block u3a-margin-left-5");
					$p = new U3A_DIV([$cb, $cblbl], null, "u3a-member-class");
				}
				else
				{
...
//				$contents1[] = new U3A_DIV([$p, $group_input, $groupname_input], null, null);
		...
		$groupname_input = new U3A_INPUT("hidden", "u3a-member-select-groupname", "u3a-member-select-groupname", null, "committee");
	}
...
//	write_log($cfg);
	...
	foreach ($cmte as $cmbr)
	{
		...
		if ($checked)
		{
			...
			$cblbl = new U3A_LABEL($cbid, $cmbr["role"], null, "u3a-member-checkbox-label-class u3a-inline-block u3a-margin-left-5");
			$p = new U3A_DIV([$cb, $cblbl], null, "u3a-member-class");
		}
		else
		{
...
//	write_log($_GET);
	...
	if ($mbr)
	{
		...
		foreach ($cmte as $cmbr)
		{
			...
			if ($addemail)
			{
				$ml = new U3A_A("#u3a-send-mail-individual-0", "contact", null, "u3a-committee-mail-class", "u3a_mail_clicked('" . $cmbr["email"] . "')");
				$ml->add_attribute("rel", "modal:open");
...
//			}
		...
		{
			$mng = [];
			$mng[] = new U3A_DIV('[su_permalink id="' . U3A_Information::u3a_manage_committee_documents_page() . '"]', "u3a-manage-committee-documents-link-div", "u3a-link-div-class");
			$mng[] = new U3A_DIV('[su_permalink id="' . U3A_Information::u3a_manage_committee_permissions_page() . '"]', "u3a-manage-committee-permissions-link-div", "u3a-link-div-class");
			if (U3A_Information::u3a_management_enabled())
			{
				$mng[] = new U3A_DIV('[su_permalink id="' . U3A_Information::u3a_manage_groups_page() . '"]', "u3a-manage-groups-link-div", "u3a-link-div-class");
				$mng[] = new U3A_DIV('[su_permalink id="' . U3A_Information::u3a_manage_members_page() . '"]', "u3a-manage-members-link-div", "u3a-link-div-class");
			}
			else
			{
			...
			$emaildiv = U3A_HTML_Utilities::get_mail_contents_div($mbr->id . "+" . $iscom[0]->id, "committee", 0, "u3a-inline-block u3a-width-70-pc u3a-height-100-pc u3a-va-top", $ndocs, $nimgs);
			$mailer = "[su_accordion]\n";
			...
			if (U3A_Committee::is_webmanager($mbr))
			{
				...
				{
					$gmdiv = new U3A_DIV($ct->email, null, "u3a-group-email-class");
...
//		$mbrs = U3A_HTML::to_html([$mbrdiv, $mdiv]);
		...
		$pgcontent .= "[/su_tabs]\n";
		$maildiv = U3A_HTML_Utilities::get_mail_contents_div($mbr->id, "individual", 0, "modal u3a-mail-dialog", $ndocs, $nimgs, "", "modal:close");
...
function u3a_manage_members_contents($atts1)
{
	...
	if ($mbr && U3A_Committee::is_committee_member($mbr))
	{
		...
		if (U3A_Committee::is_membership_secretary($mbr) || U3A_Committee::is_webmanager($mbr))
		{
			...
			$json = "TAM=1";
			$alistbtn = new U3A_BUTTON("button", "download address list", null, "u3a-wide-button", "u3a_download_address_list('$json')");
		...
		if (U3A_Committee::is_treasurer($mbr) || U3A_Committee::is_webmanager($mbr))
		{
			...
			$fromlbl = U3A_HTML::labelled_html_object("from", $frominp, "u3a-gift-aid-from-label", "u3a-gift-aid-label", false, true, "start date for list of subscription payments", false);
			$tolbl = U3A_HTML::labelled_html_object("to", $toinp, "u3a-gift-aid-to-label", "u3a-gift-aid-label", false, true, "end date for list of subscription payments", false);
...
function u3a_edit_member_contents($atts1)
{
	...
	$lbl1 = new U3A_LABEL("u3a-find-member-search-a-edit-details-edit1", "search for a member by name", null, "u3a-search-label");
	...
	$lbl2 = new U3A_LABEL("u3a-find-member-search-a-edit-details-edit2", "search for a member by number", null, "u3a-search-label");
...
function u3a_renew_member_contents($atts1)
{
	...
	$lbl1 = new U3A_LABEL("u3a-find-member-search-a-renew-member-status1", "search for a member by name", null, "u3a-search-label");
	$lbl1->add_attribute("role", "button");
	...
	$lbl2 = new U3A_LABEL("u3a-find-member-search-a-renew-member-status2", "search for a member by number", null, "u3a-search-label");
	$lbl2->add_attribute("role", "button");
...
function u3a_change_member_status_contents($atts1)
{
	...
	$lbl1 = new U3A_LABEL("u3a-find-member-search-a-change-status-status1", "search for a member by name", null, "u3a-search-label");
	$lbl1->add_attribute("role", "button");
	$mbrsearchdiv1 = new U3A_DIV([$lbl1, $mbrsearch1], "u3a-select-member-text-div-status1", "u3a-margin-bottom-5");
	$mbrdiv1 = new U3A_DIV("", "u3a-member-change-status1", "u3a-member-status-class");
	$lbl2 = new U3A_LABEL("u3a-find-member-search-a-change-status-status2", "search for a member by number", null, "u3a-search-label");
	$lbl2->add_attribute("role", "button");
...
//	$span = new U3A_SPAN("search for a member", "u3a-select-member-text-view", "u3a-inline-block");
	$lbl1 = new U3A_LABEL("u3a-find-member-search-a-view-details-view1", "search for a member by name", null, "u3a-search-label");
	$lbl1->add_attribute("role", "button");
	...
	$lbl2 = new U3A_LABEL("u3a-find-member-search-a-view-details-view2", "search for a member by number", null, "u3a-search-label");
	$lbl2->add_attribute("role", "button");
...
function u3a_delete_member_contents($atts1)
{
	...
	$lbl1 = new U3A_LABEL("u3a-find-member-search-a-delete-delete1", "search for a member by name", null, "u3a-search-label");
	$lbl1->add_attribute("role", "button");
	...
	$lbl2 = new U3A_LABEL("u3a-find-member-search-a-delete-delete2", "search for a member by number", null, "u3a-search-label");
	$lbl2->add_attribute("role", "button");
...
//		$ret1->add_attribute("disabled", "disabled");
	...
	else
	{
		...
		$ret = U3A_HTML::labelled_html_object($label . ":", $ret1, "$name-label" . $idsuffix, "u3a-label-class", false, true, null, true);
	}
	return $ret;
}
...
//		$hsel->add_attribute("disabled", "disabled");
	...
	else
	{
		...
		$title = U3A_HTML::labelled_html_object("title:", $hsel, "u3a-title-label" . $idsuffix, "u3a-title-label-class", false, true, null, true);
	}
...
//		$gsel->add_attribute("disabled", "disabled");
	...
	else
	{
		...
		$gender = U3A_HTML::labelled_html_object("gender:", $gsel, "u3a-gender-label" . $idsuffix, "u3a-gender-label-class", false, true, null, true);
	}
	$mbrdiv = new U3A_DIV([$mbrhdr, $memnum, $title, $fname, $sname, $nname, $gender], "u3a-member-div" . $idsuffix, "u3a-member-div-class u3a-border-bottom u3a-border-top");
...
//	$emergency = U3A_HTML::labelled_html_object("emergency:", $emergency1, "u3a-member-emergency-label" . $idsuffix, "u3a-emergency-label-class u3a-label-class", false, true, null, true);
	$emergency = u3a_get_member_details_form_text_field($op, "member-emergency_contact", "in emergency contact", $idsuffix, $emergency_value);
	$ctcdiv = new U3A_DIV([$ctchdr, $email, $tel, $mob, $emergency], "u3a-contact-div" . $idsuffix, "u3a-contact-div-class u3a-border-bottom");
...
//		$csel->add_attribute("disabled", "disabled");
	...
	{
		$csel = U3A_HTML_Utilities::get_county_select("u3a-member-county" . $idsuffix, "u3a-member-county-class", $county_value);
		$county = U3A_HTML::labelled_html_object("county:", $csel, "u3a-member-county-label" . $idsuffix, "u3a-county-label-class", false, true, null, true);
	}
	$adddiv = new U3A_DIV([$addhdr, $house, $address1, $address2, $address3, $town, $postcode, $county], "u3a-address-div" . $idsuffix, "u3a-address-div-class u3a-border-bottom");
...
//		$payselect->add_attribute("disabled", "disabled");
	...
	else
	{
		...
		$payment = U3A_HTML::labelled_html_object("payment type:", $payselect, "u3a-payment-type-label" . $idsuffix, "u3a-payment-type-label-class", false, true, null, true);
	}
	$gadiv = null;
...
//		$giftaid1->add_attribute("onchange", "u3a_gift_aid2_changed()");
			$giftaid2lbl = U3A_HTML::labelled_html_object("I am a UK taxpayer and I would like Shrewsbury U3A to treat all subscriptions and donations I have made in the past " .
				 "and that I make in the future as Gift Aid Donations and to reclaim tax accordingly.", $giftaid2, "u3a-member-gift_aid2-label" . $idsuffix, "u3a-join-label-class u3a-checkbox-class u3a-label-class", false, true, null, true);
			...
			$gadiv = new U3A_DIV([$gaimg, $giftaid2lbl, $gatxt1, $gatxt2, $close], "u3a_gift_aid_div" . $idsuffix, "u3a_gift_aid_div modal");
		}
		$gaspan = new U3A_SPAN("", null, "dashicons dashicons-editor-help");
		if ($op === "join" || $op === "selfedit")
		{
			$giftaid = U3A_HTML::labelled_html_object("gift aid:" . $gaspan->to_html(), $giftaid1, "u3a-member-gift_aid-label" . $idsuffix, "u3a-checkbox-class u3a-label-class", false, true, null, true);
			$giftaid->add_attribute("onclick", "u3a_join_clicked('u3a_gift_aid_div')");
		}
		else
		{
			$giftaid = U3A_HTML::labelled_html_object("gift aid:", $giftaid1, "u3a-member-gift_aid-label" . $idsuffix, "u3a-checkbox-class u3a-label-class", false, true, null, true);
		}
	}
	$tamdiv = null;
...
//		$tam1->add_attribute("disabled", "disabled");
	...
	else
	{
		...
		if ($op === "join" || $op === "selfedit")
		{
			...
			$tamlbl2 = U3A_HTML::labelled_html_object("I consent to my data being shared with The Third Age Trust", $tam2, "u3a-member-gift_TAM2-label" . $idsuffix, "u3a-join-label-class u3a-checkbox-class u3a-label-class", false, true, null, true);
			$tamdiv = new U3A_DIV([$tamtxt, $tamlbl2, $tamclose], "u3a_TAM_div" . $idsuffix, "u3a_TAM_div modal");
		}
		if ($op === "join" || $op === "selfedit")
		{
			$tam = U3A_HTML::labelled_html_object("receive TAM:" . $gaspan->to_html(), $tam1, "u3a-member-TAM-label" . $idsuffix, "u3a-checkbox-class u3a-label-class", false, true, null, true);
			$tam->add_attribute("onclick", "u3a_join_clicked('u3a_TAM_div')");
		}
		else
		{
			$tam = U3A_HTML::labelled_html_object("receive TAM:", $tam1, "u3a-member-TAM-label" . $idsuffix, "u3a-checkbox-class u3a-label-class", false, true, null, true);
		}
	}
	$nldiv = null;
...
//		$newsletter1->add_attribute("disabled", "disabled");
	...
	else
	{
		...
		if ($op === "join" || $op === "selfedit")
		{
			...
			$nllbl2 = U3A_HTML::labelled_html_object("Please send my copy of the monthly newsletter by post", $newsletter2, "u3a-member-gift_newsletter2-label" . $idsuffix, "u3a-join-label-class u3a-checkbox-class u3a-label-class", false, true, null, true);
			$nldiv = new U3A_DIV([$nltxt, $nllbl2, $nlclose], "u3a_newsletter_div" . $idsuffix, "u3a_newsletter_div modal");
		}
		if ($op === "join" || $op === "selfedit")
		{
			$newsletter = U3A_HTML::labelled_html_object("newsletter posted:" . $gaspan->to_html(), $newsletter1, "u3a-member-newsletter-label" . $idsuffix, "u3a-checkbox-class u3a-label-class", false, true, null, true);
			$newsletter->add_attribute("onclick", "u3a_join_clicked('u3a_newsletter_div')");
		}
		else
		{
			$newsletter = U3A_HTML::labelled_html_object("newsletter posted:", $newsletter1, "u3a-member-newsletter-label" . $idsuffix, "u3a-checkbox-class u3a-label-class", false, true, null, true);
		}
	}
	$affiliation = u3a_get_member_details_form_text_field($op, "member-affiliation", "other U3A membership", $idsuffix, $affiliation_value);
	$paydiv = new U3A_DIV([$payhdr, $payment, $giftaid], "u3a-payment-div" . $idsuffix, "u3a-payment-div-class u3a-border-bottom");
	$opthdr = new U3A_H(6, "Options");
	$optdiv = new U3A_DIV([$opthdr, $affiliation, $tam, $newsletter], "u3a-options-div" . $idsuffix, "u3a-options-div-class u3a-border-bottom");
	$privdiv = null;
	$amount_div = null;
//	$subs_div = null;
	if ($op === 'join')
...
//		$subs_div = new U3A_DIV($subs_due, "u3a-subs-" . $idsuffix, "u3a-subs-div-class u3a-margin-bottom-5");
	...
	elseif ($op === "view")
	{
		$btn = new U3A_BUTTON("button", "Clear", "u3a-member-btn" . $idsuffix, "u3a-member-btn-class u3a-button u3a-inline-block u3a-margin-top-5", "u3a_clear_member_form('" . $atts["op"] . $atts["suffix"] . "')");
	}
	else
	{
		$btn1 = new U3A_BUTTON("button", "OK", "u3a-member-btn" . $idsuffix, "u3a-member-btn-class u3a-button u3a-inline-block u3a-margin-top-5", "u3a_member_form('" . $atts["op"] . "')");
		if ($atts["op"] === "join")
		{
			$paybtn = new U3A_BUTTON("button", "Pay Now", "u3a-member-pay-btn" . $idsuffix, "u3a-member-pay-btn-class u3a-wide-button u3a-margin-top-5 u3a-invisible", "u3a_join_paypal()");
			$btn = [$btn1, $paybtn];
		}
		else
		{
	...
	{
		$new_member_action = new U3A_INPUT("hidden", "action", "u3a-member-action" . $idsuffix, "u3a-member-action-class", "u3a_member");
		...
		$new_member_req_val = array_key_exists($op, U3A_Information::$application_form_required_fields) ? implode(",", U3A_Information::$application_form_required_fields[$op]) : null;
		$new_member_req = new U3A_INPUT("hidden", "required", "u3a-member-required" . $idsuffix, "u3a-member-required-class", $new_member_req_val);
		$new_member_status = new U3A_INPUT("hidden", "member-status", "u3a-member-status" . $idsuffix, "u3a-member-status-class", "Provisional");
	}
	$form = new U3A_FORM([$privdiv, $amount_div, $reqtxt, $mbrdiv, $ctcdiv, $adddiv, $paydiv, $optdiv, $groups, $new_member_action, $new_member_op, $new_member_mbr, $new_member_req, $btn], "u3a_add_member", "POST", "u3a-member-form" . $idsuffix, "u3a-member-form-class");
...
//	}
	...
	$grpname = U3A_HTML::labelled_html_object("name:", $grpname1, "u3a-group-name-label" . $idsuffix, "u3a-group-name-label-class", false, true);
	$coordfname1 = new U3A_INPUT("text", "group-coordinator-forename", "u3a-group-coordinator-forename" . $idsuffix, "u3a-input-class u3a-name-input-class");
	$coordfname1->add_attribute("readonly", "readonly");
	$coordsname1 = new U3A_INPUT("text", "group-coordinator-surname", "u3a-group-coordinator-surname" . $idsuffix, "u3a-input-class u3a-name-input-class");
	$coordsname1->add_attribute("readonly", "readonly");
	$coordmnum = new U3A_INPUT("hidden", "group-coordinator-mnum", "u3a-group-coordinator-mnum" . $idsuffix, "u3a-group-coordinator-mnum-class");
	$coordsearch = do_shortcode('[u3a_find_member_dialog group="0" next_action="be_coordinator" close="tick" op="add"]');
	$coord = new U3A_DIV([$coordfname1, $coordsname1, $coordmnum], "u3a-group-coordinator-div" . $idsuffix, "u3a-group-coordinator-div-class u3a-inline-block");
	$coordname1 = U3A_HTML::labelled_html_object("coordinator:", $coord, "u3a-group-coord-label" . $idsuffix, "u3a-group-coord-label-class", false, false);
	...
	$btn->add_attribute("onclick", "on_group_button_click('$btnid')");
	...
	$venue = U3A_HTML::labelled_html_object("venue:", $venue_select, "u3a-group-venue-label" . $idsuffix, "u3a-group-venue-label-class", false, true);
	...
	$whnname = U3A_HTML::labelled_html_object("when:", $whnname1, "u3a-group-when-label" . $idsuffix, "u3a-group-when-label-class", false, false);
	...
	$maxname = U3A_HTML::labelled_html_object("maximum size:", $maxname1, "u3a-group-max-label" . $idsuffix, "u3a-group-max-label-class", false, true);
	...
	$notes = U3A_HTML::labelled_html_object("information:", $notes1, "u3a-group-notes-label" . $idsuffix, "u3a-group-notes-label-class", false, true);
	$form = new U3A_FORM([$grpid, $grpname, $coordname, $coordsearch, $venue, $whnnamediv, $whndialog, $maxname, $notes, $btn, $new_group_action], "/wp-admin/admin-ajax.php", "POST", "u3a-group-form" . $idsuffix, "u3a-group-form-class");
	return $form;
}
...
function u3a_group_select_contents($atts1)
{
	...
	$ok = new U3A_BUTTON("button", "OK", "u3a_select-group-btn" . $idsuffix, "u3a_select-group-btn-class u3a-button u3a-inline-block u3a-margin-left-5", "u3a_after_select_group('" . $atts["op"] . "')");
	$div = new U3A_DIV(U3A_HTML::labelled_html_object("group:", $sel, "u3a-select-group-label" . $idsuffix, "u3a-select-group-label-class", false, false), null, "u3a-inline-block");
...
function u3a_edit_group_form_contents($atts1)
{
	...
	$grpname1 = new U3A_INPUT("text", "group-name", "u3a-group-name" . $idsuffix, "u3a-input-class u3a-name-input-class", $group_name);
	$grpname = U3A_HTML::labelled_html_object("name:", $grpname1, "u3a-group-name-label" . $idsuffix, "u3a-group-name-label-class", false, true);
	if ($group_id)
	{
		$coorddiv = new U3A_DIV(U3A_HTML_Utilities::get_coordinators_for_group_edit($grp), "u3a-group-coordinators" . $idsuffix);
	}
	else
	{
	...
	$venue_select = new U3A_SELECT($venues, "venue", "u3a-group-venue" . $idsuffix, "u3a-group-venue-class");
	$venue = U3A_HTML::labelled_html_object("venue:", $venue_select, "u3a-group-venue-label" . $idsuffix, "u3a-group-venue-label-class", false, true);
	$whnname1 = new U3A_INPUT("text", "group-when", "u3a-group-when" . $idsuffix, "u3a-input-class u3a-when-input-class", $gmw);
	$whnname1->add_attribute("readonly", "readonly");
	$whnname = U3A_HTML::labelled_html_object("when:", $whnname1, "u3a-group-when-label" . $idsuffix, "u3a-group-when-label-class", false, false);
	$whnjson = new U3A_INPUT("hidden", "group-when-json", "u3a-group-when-json" . $idsuffix, null, $gmwj);
	$whnnamediv = new U3A_DIV([$whnname, $whnjson], "u3a-group-when-div" . $idsuffix, "u3a-when-div-class u3a-inline-block");
	$whndialog = do_shortcode('[u3a_meeting_times_dialog close="tick" op="edit" inline="yes" group="' . $group_id . '"]');
	$maxname1 = new U3A_INPUT("number", "group-max", "u3a-group-max" . $idsuffix, "u3a-input-class u3a-number-input-class", $group_max);
	$maxname1->add_attribute("min", 2);
	$maxname1->add_attribute("max", 25);
	$maxname = U3A_HTML::labelled_html_object("maximum size:", $maxname1, "u3a-group-max-label" . $idsuffix, "u3a-group-max-label-class", false, true);
...
//		$form = new U3A_FORM([$grpid, $grpname, $coordnames, $coordplus, $coordname, $venue, $whnnamediv, $whndialog, $maxname, $notes, $btn, $edit_group_action], "/wp-admin/admin-ajax.php", "POST", "u3a-group-form" . $idsuffix, "u3a-group-form-class");
	$form = new U3A_FORM([$grpid, $source, $grpname, $coorddiv, $venue, $whnnamediv, $whndialog, $maxname, $notes, $btn, $edit_group_action], "/wp-admin/admin-ajax.php", "POST", "u3a-group-form" . $idsuffix, "u3a-group-form-class");
	return $form;
}
...
function u3a_committee_mail_contents($atts1)
{
	...
	if ($atts["role"])
	{
		...
		if ($cm)
		{
			$ml = new U3A_A("mailto:" . $cm->email, $cm->role, null, "u3a-committee-mail-class");
			$ret = $ml->to_html();
		}
	}
	return $ret;
...
function u3a_role_mail_contents($atts1)
{
	...
	if ($atts["role"])
	{
		...
		if ($cm)
		{
			$ml = new U3A_A("mailto:" . $cm->email, $cm->role, null, "u3a-roles-mail-class");
			$ret = $ml->to_html();
		}
	}
	return $ret;
...
{
	return U3A_Row::count_rows("U3A_Members", ["status" => "Current", "class<>" => "System"]);
}
...
//			write_log($venue);
			if ($venue->postcode)
			{
				...
				$venues .= '<div class="u3a-venue-div"><span class="u3a-venue-label">email:</span><span class="u3a-venue-value">' . $venue->email . '</span></div>';
...
function u3a_add_member_to_group_contents($atts1)
{
	...
	else
	{
		...
		{
			$mbr = U3A_Information::u3a_logged_in_user();
			if (U3A_Group_Members::is_coordinator($mbr, $grp) || U3A_Committee::is_committee_member($mbr) || U3A_Information::u3a_has_permission($mbr, "manage group membership", $grp))
			{
				...
				$lbl1 = new U3A_LABEL("u3a-find-member-search-a-add-to-group-add2grp1", "search for a member by name", null, "u3a-search-label");
				$lbl1->add_attribute("role", "button");
				$mbrsearchdiv1 = new U3A_DIV([$lbl1, $mbrsearch1], "u3a-select-member-text-div-add2grp1", "u3a-margin-bottom-5");
				$lbl2 = new U3A_LABEL("u3a-find-member-search-a-add-to-group-add2grp2", "search for a member by number", null, "u3a-search-label");
				$lbl2->add_attribute("role", "button");
				...
				$wllbl = U3A_HTML::labelled_html_object("on waiting list", $wlcb, "u3a-add2grp-waiting-list-label", null, false, true);
...
function u3a_remove_member_from_group_contents($atts1)
{
	...
	else
	{
		...
		{
			$mbr = U3A_Information::u3a_logged_in_user();
			if (U3A_Group_Members::is_coordinator($mbr, $grp) || U3A_Committee::is_webmanager($mbr) || U3A_Information::u3a_has_permission($mbr, "manage group membership", $grp))
			{
...
//					{
						...
						{
							$gmdiv = new U3A_DIV($grpmbr->email, null, "u3a-group-email-class");
							$mailer .= $gmdiv->to_html();
						}
						$dlbtn = new U3A_BUTTON("button", "download as csv", "u3a-group-emails-download", "u3a-wide-button", "u3a_download_group_emails()");
...
function u3a_view_member_contact_details_contents($atts1)
{
	...
	if ($atts["member"])
	{
		...
		if ($mbr->email)
		{
			...
			$val0 = new U3A_SPAN($mbr->email, null, "u3a-contact-details-value u3a-inline-block");
...
function u3a_mailing_list_name_contents($atts1)
{
	...
	if ($atts["group"])
	{
		...
		if ($grp)
		{
			...
			$inp = new U3A_INPUT("text", "list-name", "u3a-mailing-list-name", "u3a-mailing-list-name-class u3a-width-10-em", $mlname);
			$mlist = U3A_HTML::labelled_html_object("mailing list name", $inp, "u3a-mailing-list-name-label", "u3a-mailing-list-name-label-class", false, false, "Enter a name for the list with no spaces or punctuation symbols except'.'.");
			$btn = new U3A_BUTTON("button", "Create", "u3a-mailing-list-create-button", "u3a-button", "u3a_create_mailing_list(" . $grp->id . ")");
...
//	write_log($atts);
	...
	if ($grp)
	{
		...
		$search_div = new U3A_DIV([$grp_members, $search_group], "u3a-find-members-search-form-div" . $idsuffix, "u3a-find-members-search-form-div-class u3a-border-top");
...
//	$search_button = new U3A_BUTTON("button", '<span class="dashicons dashicons-search"></span>', "u3a-find-members-search-button", "u3a-find-members-search-button-class u3a-button");
	...
	$search_button = new U3A_A("#", '<span class="dashicons dashicons-search"></span>', $sbid, "u3a-find-members-search-button-class");
	$search_button->add_attribute("onclick", "u3a_find_members_search_clicked('$sbid')");
	$search_button_div = new U3A_DIV($search_button, "u3a-find-members-search-button-div" . $idsuffix, "u3a-find-members-search-button-div-class");
	$search_action = new U3A_INPUT("hidden", "action", "u3a-find-members-search-action" . $idsuffix, null, "u3a_find_members_search");
	$search_next = new U3A_INPUT("hidden", "next_action", "u3a-find-members-search-next" . $idsuffix, null, $atts["next_action"]);
	...
	if ($atts["byname"] === "yes")
	{
		$surname_input = new U3A_INPUT("text", "member-surname", "find-member-surname" . $idsuffix, "find-member-surname-class find-member-input-class");
		$forename_input = new U3A_INPUT("text", "member-forename", "find-member-forename" . $idsuffix, "find-member-forename-class find-member-input-class");
		...
		$xdiv = new U3A_DIV("fill some part of name and then press " . '<span class="dashicons dashicons-search"></span>', "u3a-find-members-search-x" . $idsuffix, "u3a-find-members-search-x-class");
		$search_form = new U3A_FORM([$surname, $forename, $search_button_div, $search_action, $search_next, $search_op, $search_byname], "/wp-admin/admin-ajax.php", "POST", "u3a-find-members-search-form" . $idsuffix, "u3a-find-members-search-form-class");
	}
	else
	{
		$mnum_input = new U3A_INPUT("text", "member-number", "find-member-number" . $idsuffix, "find-member-number-class find-member-input-class");
		$mnum = U3A_HTML::labelled_html_object("membership number", $mnum_input, null, "u3a-wide-input-label-class", false, true);
		$xdiv = new U3A_DIV("enter the membership number and then press " . '<span class="dashicons dashicons-search"></span>', "u3a-find-members-search-x" . $idsuffix, "u3a-find-members-search-x-class");
		$search_form = new U3A_FORM([$mnum, $search_button_div, $search_action, $search_next, $search_op, $search_byname], "/wp-admin/admin-ajax.php", "POST", "u3a-find-members-search-form" . $idsuffix, "u3a-find-members-search-form-class");
	}
	$search_div = new U3A_DIV([$hdr1, $xdiv, $search_form], "u3a-find-members-search-form-div" . $idsuffix, "u3a-find-members-search-form-div-class u3a-border-top");
	$results_div = new U3A_DIV(null, "u3a-find-members-results-div" . $idsuffix, "u3a-find-members-results-div-class u3a-invisible");
...
// add group entry
	...
	if (U3A_Information::u3a_has_permission($member, "manage newsletters"))
	{
		...
		$tblbtn = new U3A_BUTTON("button", "download as table", "u3a-group-table-download-button", "u3a-wide-button", "u3a_download_group_table()");
	}
	if ($member)
	{
		$maildiv = U3A_HTML_Utilities::get_mail_contents_div($member->id, "individual", 0, "modal u3a-mail-dialog", 0, 0, "", "modal:close");
	}
	else
	{
		$maildiv = null;
...
function u3a_meeting_times_contents($atts1)
{
	...
	{
		$grp = U3A_Groups::get_group($atts["group"]);
		$val = json_decode($grp->meets_when, true);
		write_log($grp->meets_when, $val);
	}
	else
	{
		$val = [
...
//	write_log($atts);
	...
	$close = new U3A_A('#', $cls, 'u3a_meeting_times_a' . $idsuffix, null, "u3a_meeting_times_dialog_close('" . $atts["op"] . "');");
	$close->add_attribute("rel", "modal:close");
	$div = new U3A_DIV([$txt, $close], "u3a_meeting_times_div" . $idsuffix, "modal");
	$open = new U3A_A('#u3a_meeting_times_div' . $idsuffix, '<span class="dashicons dashicons-calendar"></span>', null, $atts['inline'] === "yes" ? "u3a-inline-block" : null);
	$open->add_attribute("rel", "modal:open");
...
function u3a_manage_committee_documents_contents($atts1)
{
	...
	if (U3A_Committee::is_committee_member($mbr))
	{
		$bck = new U3A_DIV('Back to [su_permalink id="' . U3A_Information::u3a_committee_page() . '" title="Back to Committee Page"]', "u3a-back-to-committee-link-div", "u3a-link-div-class");
		$mng .= $bck->to_html();
		$mng .= "[su_accordion]\n";
		$mng .= U3A_Information::get_manage_open_spoiler("Manage Categories", $atts["spoiler"]);
...
//			array_push($object_list, $all);
	...
	else
	{
		if ($is_committee)
		{
			$bck1 = new U3A_DIV('Back to [su_permalink id="' . U3A_Information::u3a_committee_page() . '" title="Back to Committee Page"]', "u3a-back-to-committee-link-div", "u3a-link-div-class");
...
//	var_dump($object_list);
	if ($object_list)
	{
		...
		if ($permit_types)
		{
			$sel = U3A_HTML_Utilities::get_select_list_from_object_array($object_list, "permit_to", $text_key, "id", null, null, "u3a-permit-to-list-$groups_id", "u3a-select");
			$permsel = U3A_HTML_Utilities::get_select_list_from_object_array($permit_types, "allow_to", "name", "id", null, null, "u3a-allow-to-list-$groups_id", "u3a-select");
//			$span0 = new U3A_SPAN("allow", null, "u3a-inline-block u3a-margin-right-5");
			...
			$btn0 = new U3A_BUTTON("button", "Allow", "u3a-permit-button-$groups_id", "u3a-button u3a-inline-block u3a-margin-right-5", "u3a_create_permission($groups_id, $is_committee)");
			$btn0->add_tooltip("Click to set permission shown");
			$div0 = new U3A_DIV([$btn0, $sel, $span1, $permsel], "u3a-permit-div-$groups_id", "u3a-border-bottom u3a-border-top u3a-bottom-margin-5 u3a-padding-top-5");
		}
		else
		{
			$div0 = new U3A_DIV("no permissions to give.", "u3a-permit-div-$groups_id", "u3a-border-bottom u3a-border-top u3a-bottom-margin-5 u3a-padding-top-5");
		}
	}
	else
	{
	...
	if ($perms)
	{
		...
		$btn1 = new U3A_BUTTON("button", "Remove", "u3a-remove-permit-button-$groups_id", "u3a-button u3a-inline-block u3a-margin-right-5", "u3a_remove_permission($groups_id)");
		$btn1->add_tooltip("Click to remove all selected permissions");
		$lbl1 = new U3A_LABEL("u3a-remove-permit-button-$groups_id", "selected permissions", "u3a-remove-permit-button-label-$groups_id", null);
		$divs[] = new U3A_DIV([$btn1, $lbl1], "u3a-remove-permit-button-div-$groups_id", "u3a-bottom-margin-5 u3a-top-margin-5");
...
//	var_dump($mbr);
	if ($mbr)
	{
		...
		if (U3A_Information::u3a_has_permission($mbr, "add news"))
		{
			...
			$addtitle = new U3A_INPUT("text", "news-title", "add-news-title", "u3a-add-news-title u3a-inline-block u3a-margin-bottom-5");
			...
			$addexpires->add_attribute("min", $dtf0);
			$addexpires->add_attribute("max", $dtf2);
			$addexpireslbl = new U3A_LABEL("add-news-expires", "expires:", "add-news-expires-label", "u3a-inline-block u3a-width-5em");
...
//			}
			...
			$pgcontents[] = new U3A_INPUT("hidden", "subscription-rate", "u3a-subscription-rate", null, U3A_Information::u3a_get_renewal_rate());
			$pgcontents[] = new U3A_INPUT("hidden", "associate-subscription-rate", "u3a-associate-subscription-rate", null, U3A_Information::u3a_get_renewal_rate(true));
			$renew = strtotime($mbr->renew);
			$sub_date = date("jS F Y", $renew);
			if (time() < $renew)
			{
				$pgcontents[] = new U3A_DIV(new U3A_B("Your subscription is due for renewal on $sub_date."), "u3a-paypal-text", "u3a-paypal-text-class");
			}
			else
			{
				$lapses = date("jS F Y", U3A_Information::u3a_get_membership_lapses($renew));
				$pgcontents[] = new U3A_DIV(new U3A_B("Your subscription was due for renewal on $sub_date. Please pay before $lapses"), "u3a-paypal-text", "u3a-paypal-text-class");
			}
			$pgcontents[] = new U3A_DIV("You can pay with a credit card via PayPal, you do not need to have a PayPal account.", "u3a-paypal-text", "u3a-paypal-text-class");
			$pgcontents[] = new U3A_DIV(null, "u3a-paypal-container-renew", "u3a-paypal-container");
		}
	}
	else
	{
		...
		if ($doc)
		{
			...
			$a->add_attribute("data-popup", "true");
			$lnk = $a->to_html();
			$vidurl = U3A_Row::get_single_value("U3A_Help_Videos", "url", ["name" => "registration", "category" => U3A_Help_Videos::ALLMEMBERS]);
...
function u3a_administration_contents()
{
	...
	if ($mbr && U3A_Information::u3a_has_permission($mbr->get_real_member(), "site administration"))
	{
//		$divs = [
...
//		$btndiv = new U3A_DIV($btn, "site-update-button", "u3a-margin-top-5 u3a-border-bottom");
		...
		$idinp = new U3A_INPUT("text", "newid", "u3a-site-assume-identity-input", "u3a-site-assume-identity-input-class u3a-number-input-class", $assumed_value);
		$idlbl = new U3A_LABEL("u3a-site-assume-identity-input", "membership number:", "u3a-site-assume-identity-label", "u3a-inline-block u3a-width-15-em u3a-margin-right-5");
...
//		$logh = new U3A_H(4, "Download debug log");
		...
		file_put_contents(WP_CONTENT_DIR . '/debug.log', "New log created on  " . date("d/m/Y \a\\t H:i:s") . PHP_EOL . PHP_EOL);
...
//		$tmpbutton1 = new U3A_BUTTON("button", "change docs", "u3a-change-docs-button", "u3a-button", "u3a_change_documents()");
		...
		$testalert = new U3A_BUTTON("button", "test alert", null, "u3a-wide-button", "u3a_test_alert('Test', 'a test message', 'success')");
		$testalertdiv = new U3A_DIV($testalert, "site-testalert-div", "update-type-div-class");
		$testrp = new U3A_BUTTON("button", "test reply preference", null, "u3a-wide-button", "u3a_test_reply_preference('system.test@mg.shrewsburyu3a.org.uk')");
...
function u3a_contact_page_contents($atts1)
{
	...
	$maildiv = U3A_HTML_Utilities::get_mail_contents_div(0, "contact", $atts["to"], "u3a-height-100-pc u3a-width-100-pc", 0, 0);
...
//		$ctee = U
		...
		$pgcontent .= "[/su_tab]\n";
		$div1 = new U3A_DIV('[u3a_members group="email" checked="yes"]', "u3a-all-members-email-members-list", "u3a-inline-block u3a-width-30-pc u3a-height-100-pc u3a-va-top");
		$ndocs = U3A_Row::count_rows("U3A_Documents", ["groups_id" => 0, "document_type" => U3A_Documents::PRIVATE_DOCUMENT_TYPE]);
		$nimgs = U3A_Row::count_rows("U3A_Documents", ["groups_id" => 0, "document_type" => U3A_Documents::COMMITTEE_IMAGE_TYPE]);
		$emaildiv = U3A_HTML_Utilities::get_mail_contents_div($mbr->id . "+" . $ctee->id, "individual", 0, "u3a-inline-block u3a-width-70-pc u3a-height-100-pc u3a-va-top", $ndocs, $nimgs);
...
//	$atts = shortcode_atts(array(), $atts1, 'u3a_list_all_members');
	...
	if (U3A_Committee::is_committee_member($mbr))
	{
		...
		{
			$links[] = new U3A_A('#', $letter, "u3a-member-initial-" . $letter, "u3a-member-initial-letter-class u3a-inline-block u3a-margin-right-2", "u3a_initial_letter_clicked('" . $letter . "')");
		}
		$links[] = new U3A_A('#', "all", "u3a-member-initial-all", "u3a-member-initial-letter-class u3a-inline-block u3a-margin-left-10", "u3a_initial_letter_clicked('all')");
		...
		$tam = U3A_HTML::labelled_html_object("Third Age Matters", new U3A_DIV(U3A_INPUT::yes_no_ignore_radio_array("TAM"), null, "u3a-inline-block"), null, "u3a-width-50-pc", false, true, null);
		$nl = U3A_HTML::labelled_html_object("posted newsletter", new U3A_DIV(U3A_INPUT::yes_no_ignore_radio_array("newsletter"), null, "u3a-inline-block"), null, "u3a-width-50-pc", false, true, null);
		$ga = U3A_HTML::labelled_html_object("gift aid", new U3A_DIV(U3A_INPUT::yes_no_ignore_radio_array("gift_aid"), null, "u3a-inline-block"), null, "u3a-width-50-pc", false, true, null);
		$labval = [
			"Cheque"		 => "Cheque",
			"Cash"		 => "Cash",
			"PayPal"		 => "PayPal",
		...
		$pt = U3A_HTML::labelled_html_object("payment type", new U3A_DIV(U3A_INPUT::checkbox_array("payment_type", $labval, false, "u3a-width-8-em"), null, "u3a-inline-block"), null, "u3a-width-50-pc", false, true, null);
		$em = U3A_HTML::labelled_html_object("email address", new U3A_DIV(U3A_INPUT::yes_no_ignore_value_radio_array("email"), null, "u3a-inline-block"), null, "u3a-width-50-pc", false, true, null);
		...
		foreach (U3A_Members::$display_column_names as $label => $colname)
		{
			if (is_array($colname))
			{
				...
				$lbl = U3A_HTML::labelled_html_object($label, $inp, "u3a-members-label-" . $cname, "u3a-member-column-label-class u3a-label-class", false, false);
				$btnup = new U3A_A('#', '<span class="dashicons dashicons-arrow-up-alt2"></span>', "u3a-members-column-$cname-up", "u3a-invisible", "u3a_toggle_up_down('u3a-members-column-$cname-', 'up', '$divid')");
				$btndown = new U3A_A("#", '<span class="dashicons dashicons-arrow-down-alt2"></span>', "u3a-members-column-$cname-down", "", "u3a_toggle_up_down('u3a-members-column-$cname-', 'down', '$divid')");
				...
				foreach ($colname as $label1 => $colname1)
				{
					if ($label1 !== 'default')
					{
						$inp1 = new U3A_INPUT("checkbox", $colname1, "u3a-members-sub-" . $colname1, "u3a-members-column-checkbox-class", $cname . '/' . $colname1);
						$inp1->add_attribute("onchange", "u3a_inner_clicked('u3a-members-', '$cname')");
						$lbl1 = U3A_HTML::labelled_html_object($label1, $inp1, "u3a-members-label-" . $colname1, "u3a-member-column-label-class u3a-label-class", false, true);
						$sub[] = $lbl1;
					}
				}
			...
			else
			{
				...
				$cols[] = U3A_HTML::labelled_html_object($label, $inp, "u3a-members-label-" . $colname, "u3a-member-column-label-class u3a-label-class", false, true);
			}
		}
...
//		$coldiv = new U3A_DIV([$cols, $colclose], "u3a-member-select-div", "u3a-member-select-div-class u3a-height-60-pc u3a-overflow-y-auto");
		...
		$sortdiv1 = new U3A_DIV([$sorthdr, $sortlist], "u3a-members-column-sort-list-container", "u3a-member-column-div-class u3a-height-90-pc  u3a-overflow-y-auto u3a-margin-bottom-10");
		$sortdiv = new U3A_DIV([$sortdiv1, $sortclose], "u3a-members-column-sort-list-outer", "u3a-padding-5 modal u3a-height-40-pc");
		$btn_display = new U3A_BUTTON("button", "display table", "u3a-members-display-table", "u3a-mid-button", "u3a_members_display_table()");
		$btn_display->add_attribute("disabled", "disabled");
		$btn_csv = new U3A_BUTTON("button", "download csv", "u3a-members-download-csv", "u3a-mid-button u3a-margin-left-5", "u3a_members_download('csv')");
		$btn_csv->add_attribute("disabled", "disabled");
		$btn_xls = new U3A_BUTTON("button", "download xlsx", "u3a-members-download-xlsx", "u3a-mid-button u3a-margin-left-5", "u3a_members_download('xlsx')");
		$btn_xls->add_attribute("disabled", "disabled");
		$display_page = new U3A_INPUT("hidden", null, "u3a-display-members-page-link", null, get_permalink(U3A_Information::u3a_members_display_page()));
		...
		$rhs = new U3A_DIV([$filter_heading, $tam, $nl, $ga, $pt, $em, $display_div, $coldiv, $sortdiv], "u3a-all-members-filter", "u3a-border-left u3a-inline-block u3a-width-40-pc u3a-padding-left-5 u3a-all-members-div-class");
...
// add group entry
			...
			$mbrdiv = new U3A_DIV('[u3a_members group="' . $grp->id . '" checked="no"]', "u3a-group-members-div-" . $grp->id, "u3a-member-list-class");
...
function u3a_help_page_contents($atts1)
{
	...
	if (U3A_Committee::is_committee_member($mbr) || U3A_Group_Members::is_a_coordinator($mbr))
	{
		...
		$cat2 = U3A_Document_Categories::get_category_id("Website User Guide", U3A_Documents::USERGUIDE_COORDINATORS_DOCUMENT_TYPE);
		$cat3 = U3A_Document_Categories::get_category_id("Website User Guide", U3A_Documents::USERGUIDE_COMMITTEE_DOCUMENT_TYPE);
...
function u3a_application_contents($atts)
{
	...
	$rate = new U3A_INPUT("hidden", "subscription-rate", "u3a-subscription-rate", null, U3A_Information::u3a_get_current_join_rate());
	$arate = new U3A_INPUT("hidden", "associate-subscription-rate", "u3a-associate-subscription-rate", null, U3A_Information::u3a_get_current_join_rate(true));
	...
	$text = new U3A_DIV("You can pay with a credit card via PayPal, you do not need to have a PayPal account.", "u3a-paypal-text", "u3a-paypal-text-class u3a-invisible");
